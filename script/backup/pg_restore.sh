...
# Примеры:
#   ./pg_restore.sh account /path/to/account_backup_20250112_030000.tar.gz
...
if [[ $# -ne 2 ]]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  instance     - PostgreSQL instance name (account, authorization, employee, organization, content, tg-bot)"
    echo "  backup_file  - Path to backup file (*.tar.gz)"
    echo ""
    echo "Example:"
    echo "  $0 account backups/postgresql/account/account_backup_20250112_030000.tar.gz"
    exit 1
fi
